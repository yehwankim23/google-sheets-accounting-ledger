...
function createNextMonth() {
  ...
  newSheet.getRange("A1").setValue(`${newName} 회계 보고`);
  let maxRows = previousSheet.getMaxRows();
  newSheet.getRange("A3").setValue(month);
  newSheet.getRange("B3").setValue(1);
...
function onEdit(e) {
  ...
  sheet.getRange(maxRows, 6).setValue(`=D${maxRows}-E${maxRows}`);
}
